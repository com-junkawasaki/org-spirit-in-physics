<script lang="ts">
  ...
  import { normalizeEmotionName, EMOTION_KEYS } from '../utils/emotion-normalization';
  import { JUNG_STIMULUS_WORDS } from '../utils/jung';
  
  // Timeline client interface (passed from parent app)
  interface TimelineClientInterface {
    getIntegratedTimeline: (req: { participantId: string; sessionId?: string }) => Promise<any>;
    getWordStatistics: (req: { participantId: string; sessionId?: string }) => Promise<any>;
    getEmotionVectors: (req: { participantId: string; sessionId?: string }) => Promise<any>;
    getWordAggregates: (req: { participantId: string; sessionId?: string }) => Promise<any>;
    getAnalysis: (req: { participantId: string; sessionId?: string }) => Promise<any>;
  }
  ...
  let {
    ...
    forceMode,
    timelineClient
  }: TimelineVisualizationProps & { timelineClient: TimelineClientInterface } = $props();

  // State
  let data: TimelineDataPoint[] = $state([]);
  let wordStatistics: WordStatisticsData[] = $state([]);
  let emotionVectors: EmotionVectorData[] = $state([]);
  let wordAggregates: WordAggregateData[] = $state([]);
