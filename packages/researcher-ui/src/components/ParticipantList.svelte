<script lang="ts">
  // Participant type definition (minimal for this component)
  interface ParticipantData {
    id: string;
    age?: number;
    gender?: string;
    isPublic?: boolean;
    createdAt?: { seconds: bigint | number } | null;
  }

  let {
    participants,
    getParticipantUrl = (id: string) => `/researcher/participants/${id}`,
    basePath = '/researcher'
  } = $props<{
    participants: ParticipantData[];
    getParticipantUrl?: (id: string) => string;
    basePath?: string;
  }>();
  ...
  const filteredParticipants = $derived(
    participants.filter((p: ParticipantData) =>
      ...
      <tbody>
        {#each filteredParticipants as p}
          <tr>
            ...
            <td>
              <a href={getParticipantUrl(p.id)} class="id-text hover:underline">{p.id}</a>
            </td>
            <td>{p.age ?? '---'}</td>
            <td>{p.gender ?? '---'}</td>
            <td>
            ...
            <td>
              <div class="row-actions">
                <a href={getParticipantUrl(p.id)} title="詳細" class="action-btn">👁️</a>
