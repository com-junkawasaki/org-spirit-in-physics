-- Merkle DAG: enum_conversion_complete -> recreate_views_after_enum
-- ENUM型変換後にビューとマテリアライズドビューを再作成

-- 1. マテリアライズドビューを再作成（file_typeがENUM型になったため）
...
SELECT 
  ...
  -- モダリティ別集約（サブクエリで事前集約）
  COALESCE(
    (SELECT json_object_agg(file_type::text, total_score)
     FROM (
       SELECT 
         tee2.file_type,
...
-- 2. ビューを再作成（handednessがENUM型になったため）
...
SELECT
  ...
  p.age,
  p.gender,
  p.handedness::text as handedness,
  s.session_type,
  s.session_index,
  s.start_time,
  s.end_time,
