...
async def list_participant_directories(ctx: WorkflowActivityContext, dataset_path_str: str) -> List[Dict[str, str]]:
    """Activity to list participant directories."""
...
async def process_participant(ctx: WorkflowActivityContext, input_data: Dict[str, Any]) -> Dict[str, Any]:
    """Activity to process a single participant."""
    participant_id = input_data["participant_id"]
    participant_path_str = input_data["participant_path"]
...
async def process_session(ctx: WorkflowActivityContext, input_data: Dict[str, Any]) -> Dict[str, Any]:
    """Activity to process a session."""
    participant_id = input_data["participant_id"]
    participant_path_str = input_data["participant_path"]
    participant_path = Path(participant_path_str)
...
async def generate_timeline(ctx: WorkflowActivityContext, input_data: Dict[str, Any]) -> Dict[str, Any]:
    """Activity to generate timeline."""
    participant_id = input_data["participant_id"]
    session_id = input_data["session_id"]
    session_index = input_data.get("session_index", 0)
    start_ts = input_data["start_ts"]
    end_ts = input_data.get("end_ts")
...
async def process_physiological_data(ctx: WorkflowActivityContext, input_data: Dict[str, Any]) -> Dict[str, Any]:
    """Activity to process physiological data."""
    participant_id = input_data["participant_id"]
    session_id = input_data["session_id"]
    participant_path_str = input_data["participant_path"]
    participant_path = Path(participant_path_str)
...
async def generate_stimulus_audio(ctx: WorkflowActivityContext, input_data: Dict[str, Any]) -> Dict[str, Any]:
    """Activity to generate stimulus audio."""
    word_id = input_data["id"]
