...
func (h *ParticipantHandler) GetStimulusWords(
	...
	for _, w := range words {
		resp.Words = append(resp.Words, &participantv1.StimulusWord{
			...
			Japanese:      w.Japanese,
			English:       w.English,
			French:        pgTextToString(w.French),
			Spanish:       pgTextToString(w.Spanish),
			Russian:       pgTextToString(w.Russian),
			Arabic:        pgTextToString(w.Arabic),
			Chinese:       pgTextToString(w.Chinese),
...
func (h *ParticipantHandler) GetStimulusWord(
	...
	resp := &participantv1.GetStimulusWordResponse{
		Word: &participantv1.StimulusWord{
			...
			French:        pgTextToString(word.French),
			Spanish:       pgTextToString(word.Spanish),
			Russian:       pgTextToString(word.Russian),
			Arabic:        pgTextToString(word.Arabic),
			Chinese:       pgTextToString(word.Chinese),
