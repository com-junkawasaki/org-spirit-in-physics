-- Define missing enum types and update columns with lowercase support
...
-- 2. emotion_name_enum (include both capitalized and lowercase)
DROP TYPE IF EXISTS emotion_name_enum_old CASCADE;
ALTER TYPE emotion_name_enum RENAME TO emotion_name_enum_old;
...
CREATE TYPE emotion_name_enum AS ENUM (
    ...
    'Envy', 'Excited', 'Guilt', 'Horror', 'Interest', 'Joy', 'Love', 'Nostalgia', 
    'Pain', 'Pride', 'Realization', 'Relief', 'Romance', 'Sadness', 'Satisfaction', 'Shame', 
    'Surprise (negative)', 'Surprise (positive)', 'Sympathy', 'Tiredness', 'Triumph'
);

-- Update columns to use new ENUM
ALTER TABLE session_events 
ALTER COLUMN event_type TYPE session_event_type_enum USING event_type::text::session_event_type_enum;

ALTER TABLE timeline_emotion_entries 
ALTER COLUMN emotion_name TYPE emotion_name_enum USING emotion_name::text::emotion_name_enum;
