...
**Deferred — no viewer is landed with this ADR.** The branch this ADR came from
...
- the Canvas 2D viewer cannot be the authoritative 3D surface; Canvas 2D is
  ...
  degraded fallbacks
- the Three.js variant introduces a second 3D engine from a CDN, which the rule
  names as prohibited
- the KAMI variant is KAMI only in name. It attempts
  `import("./kami-web/kamiWeb.js")`, a file that exists neither on that branch
  nor here, so the bridge never loads ("KAMI bridge not found"); it then renders
  through hand-written `navigator.gpu` / WGSL / `beginRenderPass` with a Canvas
  2D fallback — the per-app duplication of raw GPU state the rule forbids

The research contract in this document is renderer-independent, and everything
it depends on — the sample manifest, the generator, and the method — is landed.
A conforming viewer built on the canonical kami-engine stack is the remaining
work; see PR #24 for the prototypes.
