  ...
  try {
    ...
    switch (action) {
      case 'analyze-single-workflow':
        ...
        const response = await fetch(`${origin}/api/participant/admin/emotion-analysis?action=analyze-single&participantId=${participantId}&videoFile=${videoFile}&sessionType=${sessionType}`);
      ...
      case 'analyze-batch-workflow':
        ...
        try {
          const batchResult = await analyzeAllParticipantVideos(participantId);
