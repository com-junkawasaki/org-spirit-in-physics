<script lang="ts">
	...
	function renderTimeline() {
		...
		const reactionTimeExtent = d3.extent(filteredDataSorted, d => {
			const val = typeof d.reactionTime === 'number' && !isNaN(d.reactionTime) ? d.reactionTime : 0;
			return val;
		}) as [number, number];
		if (reactionTimeExtent[0] === reactionTimeExtent[1]) {
			reactionTimeExtent[1] = reactionTimeExtent[0] + 1000;
		}
		const reactionTimeScale = d3.scaleLinear()
			.domain(reactionTimeExtent)
		...
		if (filters.wordDisplay && filters.showWordLabels) {
			g.selectAll('.word-label')
				...
				.on('mouseover', function(event, d) {
					if (onDataPointSelect) onDataPointSelect(d);
					if (onTooltipShow) onTooltipShow(event as MouseEvent, d);
				})
				.on('mouseout', function() {
					if (onDataPointSelect) onDataPointSelect(null);
		...
		if (filters.reactionValues) {
			g.selectAll('.reaction-value-point')
				...
				.attr('cy', d => reactionValueScale(d.reactionValue || 0))
				.attr('r', 3)
				.style('fill', '#2563eb')
				.style('stroke', '#fff')
				.style('stroke-width', 1)
		...
		if (filters.reactionTime) {
			g.selectAll('.reaction-time-point')
				...
				.attr('cy', d => reactionTimeScale(d.reactionTime || 0))
				.attr('r', 3)
				.style('fill', '#dc2626')
				.style('stroke', '#fff')
				.style('stroke-width', 1)
		...
		if (filters.physiologicalThreshold) {
			g.selectAll('.physiological-point')
				.data(filteredDataSorted.filter(d => {
					...
					return Array.isArray(p) ? p.length > 0 : typeof p === 'object' && p != null;
				}))
				.enter()
				.append('circle')
		...
		if (filters.showEmotionDetails) {
			const legend = g.append('g')
				...
				.attr('transform', `translate(${innerWidth - 190}, 20)`);
		...
		g.append('g')
			.attr('class', 'grid')
			.call(d3.axisLeft(reactionValueScale as any)
				.tickSize(-innerWidth)
				.ticks(3)
				.tickFormat(() => '')
			)
