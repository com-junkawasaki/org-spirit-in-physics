...
export function calculateWordDistances(results: WordResponseData[]): WordDistancePair[] {
	...
	const physiologicalValues = results.map(r => {
		if (Array.isArray(r.physiologicalData)) {
			return r.physiologicalData.reduce((sum, val) => sum + Math.abs(val as number), 0) / r.physiologicalData.length;
		}
	...
	for (let i = 0; i < words.length; i++) {
		for (let j = i + 1; j < words.length; j++) {
			...
			const avgPh1 = results1.reduce((sum, r) => {
				if (Array.isArray(r.physiologicalData)) {
					return sum + r.physiologicalData.reduce((s, v) => s + Math.abs(v as number), 0) / r.physiologicalData.length;
				}
