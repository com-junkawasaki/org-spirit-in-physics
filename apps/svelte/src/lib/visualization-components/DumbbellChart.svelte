<script lang="ts">
	...
	$effect(() => {
		if (browser && d3 && svgRef) {
			renderDumbbellChart();
		}
	});

	function prepareDumbbellData(): DumbbellDataPoint[] {
		...
		return Object.entries(wordGroups)
			.map(([word, points]) => {
				...
				return {
					word,
					firstHalf: {
						avgReactionTime: firstHalf.length > 0
							? firstHalf.reduce((sum, d) => sum + (d.reactionTime || 0), 0) / firstHalf.length
							: 0,
						avgReactionValue: firstHalf.length > 0
							? firstHalf.reduce((sum, d) => sum + (d.reactionValue || 0), 0) / firstHalf.length
							: 0,
						count: firstHalf.length
					},
					secondHalf: {
						avgReactionTime: secondHalf.length > 0
							? secondHalf.reduce((sum, d) => sum + (d.reactionTime || 0), 0) / secondHalf.length
							: 0,
						avgReactionValue: secondHalf.length > 0
							? secondHalf.reduce((sum, d) => sum + (d.reactionValue || 0), 0) / secondHalf.length
							: 0,
	...
	function renderDumbbellChart() {
		...
		const maxValue = d3.max(dumbbellData, d =>
			Math.max(d.firstHalf.avgReactionValue, d.secondHalf.avgReactionValue)
		) || 1;
...
<svg bind:this={svgRef} width={width} height={400} class="border"></svg>
