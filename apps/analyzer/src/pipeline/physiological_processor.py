...
class PhysiologicalProcessor:
    ...
    def load_skin_potential_data(self, response_id: str, terminusdb_client) -> List[Dict[str, Any]]:
        """
        TerminusDBから皮膚電位の時系列データを読み込み
        """
        logging.info(f"Loading skin potential data for response {response_id}")

        from terminusdb_client import WOQLQuery
        query = WOQLQuery().woql_and(
            WOQLQuery().triple("v:Timeseries", "rdf:type", "scm:SkinPotentialTimeseries"),
            WOQLQuery().triple("v:Timeseries", "scm:response_id", WOQLQuery().string(response_id)),
            WOQLQuery().triple("v:Timeseries", "scm:timestamp_offset_ms", "v:Timestamp"),
            WOQLQuery().triple("v:Timeseries", "scm:value", "v:Value")
        ).order_by("v:Timestamp")

        response = terminusdb_client.query(query)

        if response.get('bindings'):
            data = response.get('bindings', [])
            logging.info(f"Loaded {len(data)} skin potential data points")
            # Convert TerminusDB format to expected format
            return [{
                'timestamp_offset_ms': item.get('Timestamp', {}).get('@value'),
