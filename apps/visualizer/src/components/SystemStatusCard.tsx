      ...
      if (response.ok) {
        ...
        const transformedStatus: SystemStatus = {
          ...
          services: data.health?.services?.map((service: { name: string; status: string; responseTime: number }) => ({
            ...
            details: {
              database: service.name === 'Supabase' ? process.env.NEXT_PUBLIC_SUPABASE_URL?.replace('https://', '').split('.')[0] || 'supabase' : undefined,
              ...
              <div className="text-right">
                ...
                {service.details && (
                  <div className="mt-2 text-xs text-muted-foreground">
                    {service.service === 'Supabase' && service.details.database && (
