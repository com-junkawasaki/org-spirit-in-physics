...
export default function ProcessSessionsPage() {
  ...
  async function runSessionImport() {
    ...
    try {
      setLogLines(l => [...l, 'セッションデータインポート開始...'])
      ...
      setLogLines(l => [...l, `インポート完了: ${data.summary?.importedSessions || 0}件のセッションをインポート`])
  ...
  return (
    <div className="p-4 space-y-4">
      <h1 className="text-2xl font-bold">セッションデータインポート</h1>
      
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <h2 className="font-semibold text-green-800 mb-2">プロセス概要</h2>
        <p className="text-green-700 text-sm">
          参加者ディレクトリから session_data.json ファイルを読み取り、セッションイベントと単語応答データをNeo4jにインポートします。
          また、2025-08を含むCSVファイルを生理データとしてインポートします。
        </p>
      </div>
