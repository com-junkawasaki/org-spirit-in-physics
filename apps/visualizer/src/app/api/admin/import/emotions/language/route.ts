...
// Merkle DAG: import.emotions.language -> language_emotion_data_import
// Language感情データインポートAPIエンドポイント
// 依存関係: @participants/ (dataset), neo4j

export async function POST(request: NextRequest) {
  try {
    console.log('API: Starting language emotion import from dataset...');
    ...
    // Merkle DAG: import.emotions.language.check_participant
    // 参加者存在確認
    ...
    // Merkle DAG: import.emotions.language.resolve_dataset_path
    // データセットパス解決
    ...
    // Merkle DAG: import.emotions.language.find_hume_data
    // HumeAIデータディレクトリ検索
    ...
    // Merkle DAG: import.emotions.language.find_registry_file
    // Registryファイル検索
    ...
    // Merkle DAG: import.emotions.language.process_language_csv
    // Language CSVファイル処理
    const csvDir = path.join(targetRegistry.path, 'csv', registryUuid);
    const languageCsvPath = path.join(csvDir, 'language.csv');

    try {
      await fs.access(languageCsvPath);
    } catch {
      return NextResponse.json({
        success: false,
        error: 'language.csv file not found'
      }, { status: 404 });
    }

    const csvContent = await fs.readFile(languageCsvPath, 'utf-8');
    const languageData = await processLanguageCSV(csvContent, participantId, sessionId, registryUuid);

    if (languageData.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No language data found in CSV file'
      }, { status: 400 });
    }

    // Merkle DAG: import.emotions.language.bulk_insert
    // バルク挿入でNeo4jに保存
    await client.bulkInsertNodes('EmotionAnalysis', languageData, 1000);

    // Merkle DAG: import.emotions.language.create_relationships
    // リレーションシップ作成
    const relationshipQuery = `
      MATCH (s:ExperimentSession {id: $sessionId})
      MATCH (ea:EmotionAnalysis {session_id: $sessionId, file_type: 'language'})
      MERGE (s)-[:HAS_EMOTION_ANALYSIS]->(ea)
      RETURN count(ea) as relationship_count
    `;
    await client.query(relationshipQuery, { sessionId });

    console.log(`API: Language emotion import completed for participant ${participantId}, session ${sessionId}`);
    ...
    return NextResponse.json({
      ...
      registryUuid,
      statistics: {
        languageEntries: languageData.length,
        emotionsProcessed: languageData.reduce((sum, entry) => sum + (entry.emotions?.length || 0), 0)
      }
    });

  } catch (error) {
    console.error('API: Failed to import language emotions:', error);
    return NextResponse.json({
      success: false,
...
// Merkle DAG: import.emotions.language.scan_registry_files
// Registryファイルスキャン関数
...
// Merkle DAG: import.emotions.language.process_csv
// Language CSVファイル処理関数
async function processLanguageCSV(content: string, participantId: string, sessionId: string, registryUuid: string) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (line) {
      ...
      const rowData: any = {
        id: `language_${participantId}_${sessionId}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
