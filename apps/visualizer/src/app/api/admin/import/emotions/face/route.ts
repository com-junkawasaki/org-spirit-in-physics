...
// Merkle DAG: import.emotions.face -> face_emotion_data_import
// Face感情データインポートAPIエンドポイント
// 依存関係: @participants/ (dataset), neo4j

export async function POST(request: NextRequest) {
  try {
    console.log('API: Starting face emotion import from dataset...');
    ...
    // Merkle DAG: import.emotions.face.check_participant
    // 参加者存在確認
    ...
    // Merkle DAG: import.emotions.face.resolve_dataset_path
    // データセットパス解決
    ...
    // Merkle DAG: import.emotions.face.find_hume_data
    // HumeAIデータディレクトリ検索
    ...
    // Merkle DAG: import.emotions.face.find_registry_file
    // Registryファイル検索
    ...
    // Merkle DAG: import.emotions.face.process_face_csv
    // Face CSVファイル処理
    const csvDir = path.join(targetRegistry.path, 'csv', registryUuid);
    const faceCsvPath = path.join(csvDir, 'face.csv');

    try {
      await fs.access(faceCsvPath);
    } catch {
      return NextResponse.json({
        success: false,
        error: 'face.csv file not found'
      }, { status: 404 });
    }

    const csvContent = await fs.readFile(faceCsvPath, 'utf-8');
    const faceData = await processFaceCSV(csvContent, participantId, sessionId, registryUuid);

    if (faceData.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'No face data found in CSV file'
      }, { status: 400 });
    }

    // Merkle DAG: import.emotions.face.bulk_insert
    // バルク挿入でNeo4jに保存
    await client.bulkInsertNodes('EmotionAnalysis', faceData, 1000);

    // Merkle DAG: import.emotions.face.create_relationships
    // リレーションシップ作成
    const relationshipQuery = `
      MATCH (s:ExperimentSession {id: $sessionId})
      MATCH (ea:EmotionAnalysis {session_id: $sessionId, file_type: 'face'})
      MERGE (s)-[:HAS_EMOTION_ANALYSIS]->(ea)
      RETURN count(ea) as relationship_count
    `;
    await client.query(relationshipQuery, { sessionId });

    console.log(`API: Face emotion import completed for participant ${participantId}, session ${sessionId}`);
    ...
    return NextResponse.json({
      ...
      registryUuid,
      statistics: {
        faceEntries: faceData.length,
        emotionsProcessed: faceData.reduce((sum, entry) => sum + (entry.emotions?.length || 0), 0)
      }
    });

  } catch (error) {
    console.error('API: Failed to import face emotions:', error);
    return NextResponse.json({
      success: false,
...
// Merkle DAG: import.emotions.face.scan_registry_files
// Registryファイルスキャン関数
...
// Merkle DAG: import.emotions.face.process_csv
// Face CSVファイル処理関数
async function processFaceCSV(content: string, participantId: string, sessionId: string, registryUuid: string) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (line) {
      ...
      const rowData: any = {
        id: `face_${participantId}_${sessionId}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
