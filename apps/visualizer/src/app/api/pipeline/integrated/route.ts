...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // BPMN: Task_SessionImport - セッションデータインポート
      const sessionImportPromise = importSessionData(participantId, pipelineOptions);
      
      // BPMN: Task_EmotionImport - 感情データインポート
      const emotionImportPromise = importEmotionData(participantId, pipelineOptions);
      
      // BPMN: Task_PhysiologicalImport - 生理データインポート
      const physiologicalImportPromise = importPhysiologicalData(participantId, pipelineOptions);
      ...
      const dataValidation = await validateImportedData(participantId);
      ...
      const physiologicalAnalysisPromise = performPhysiologicalAnalysis(participantId, pipelineOptions);
      ...
      const featureExtraction = await extractFeatures(participantId, {
      ...
      const spiritProbability = await calculateSpiritProbability(participantId, featureExtraction);
      ...
      const resultsAggregation = await aggregateResults(participantId, {
      ...
      const statisticsUpdate = await updateStatistics(participantId, resultsAggregation);
