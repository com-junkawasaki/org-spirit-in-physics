...
// セッションデータインポート処理関数（import-service経由）
async function importSessionsFromDataset() {
  try {
    // Merkle DAG: import.sessions.call_service
    // Import service経由でセッションデータをインポート
    const response = await fetch(`${IMPORT_SERVICE_URL}/import/sessions`, {
      method: 'POST',
      headers: {
