...
// 感情分析データインポート処理関数（import-service経由）
async function importEmotionsFromDataset() {
  try {
    // Merkle DAG: import.emotions.call_service
    // Import service経由で感情データをインポート
    const response = await fetch(`${IMPORT_SERVICE_URL}/import/emotions`, {
      method: 'POST',
      headers: {
